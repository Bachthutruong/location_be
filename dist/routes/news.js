...
router.post('/', authenticate, authorize(UserRole.ADMIN, UserRole.STAFF, UserRole.MANAGER), upload.array('images', 20), async (req, res) => {
    try {
        ...
        if (files && files.length > 0) {
            try {
                imageUrls = await Promise.all(files.map(file => uploadToCloudinary(file, 'location-management/news')));
            }
            catch (uploadError) {
...
router.put('/:id', authenticate, authorize(UserRole.ADMIN, UserRole.STAFF, UserRole.MANAGER), upload.array('images', 20), async (req, res) => {
    try {
        const news = await News.findById(req.params.id);
        if (!news) {
            return res.status(404).json({ message: 'News not found' });
