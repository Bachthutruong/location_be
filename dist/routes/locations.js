...
router.post('/import', authenticate, authorize(UserRole.ADMIN, UserRole.STAFF, UserRole.MANAGER), upload.single('file'), async (req, res) => {
    try {
        ...
        for (let r = 0; r < dataRows.length; r++) {
            ...
            const get = (name) => (colIndex(name) >= 0 && row[colIndex(name)] !== undefined ? row[colIndex(name)].trim() : '');
            const name = get('name');
            ...
            try {
                ...
                errors.push({ row: r + 2, message: 'Google 地圖連結無效' });
                continue;
            }
...
router.get('/manager/my', authenticate, authorize(UserRole.MANAGER), async (req, res) => {
    try {
...
router.get('/manager/:id', authenticate, authorize(UserRole.MANAGER), async (req, res) => {
    try {
        ...
        const managerId = location.manager?._id
            ? String(location.manager._id)
...
router.post('/', authenticate, authorize(UserRole.MANAGER, UserRole.ADMIN), upload.array('images', 10), async (req, res) => {
    try {
        ...
        if (isAdmin) {
            location.approvedBy = req.user.id;
...
router.put('/:id', authenticate, upload.array('images', 10), async (req, res) => {
    try {
        ...
        if (googleMapsLink)
            location.googleMapsLink = googleMapsLink.trim();
        if (description)
        ...
        if (rawKeep) {
            try {
                keepImages = Array.isArray(rawKeep) ? rawKeep : JSON.parse(rawKeep);
            }
            catch {
...
router.patch('/:id/approve', authenticate, authorize(UserRole.STAFF, UserRole.ADMIN), async (req, res) => {
    try {
        const location = await Location.findById(req.params.id);
        if (!location) {
            return res.status(404).json({ message: 'Location not found' });
...
router.patch('/:id/reject', authenticate, authorize(UserRole.STAFF, UserRole.ADMIN), async (req, res) => {
    try {
        const location = await Location.findById(req.params.id);
        if (!location) {
            return res.status(404).json({ message: 'Location not found' });
